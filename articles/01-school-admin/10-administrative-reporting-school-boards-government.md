...
Principal spends hours compiling data for board meetings: What's our student retention rate? How much have we spent on infrastructure? What's our average
...
- Board members make decisions without complete information
...
1. Create a data dictionary: define what data to track (enrollment, fees, staff strength, exam scores)
...
- Use visual dashboards (charts, not just tables) for board meetings
...
A: Enrollment (total, new, withdrawn), finances (fees collected, expenses, balance), academics (attendance, major events).

Q: What's UDISE+ and when is it due?
A: UDISE+ (Unified District Information System for Education Plus) is India's education data system. Annual submission by Sep 30. Ask your state IT cell.

Q: How do we ensure data accuracy in reports?
A: Assign a data steward, verify data 3 times, use validated sources (not rough notes).

Q: Can we use dashboards for board meetings?
A: Yes. Visual dashboards (charts, KPIs) are more compelling than tables. Update them in real-time.
